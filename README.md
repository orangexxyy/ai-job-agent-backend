...
- Step 27C：新增本地 Profile Draft Apply 工具，默认 dry-run，确认后备份并应用正式 candidate_profile
- Step 27.5：统一 Demo 主线端口为 8002，强制 Profile Draft 交互确认，并过滤已过期的 available 面试时间
- Step 28A：为用户确认且验证成功的 Profile Draft Apply 增加独立的本地最小操作留痕
- Step 29A：新增本地 Profile Draft Review API 和前端审核区，二次确认后复用同一套 apply/history 安全流程
...
访问 `http://127.0.0.1:5173`。页面包含 Final Send Gate、Action History 和本地 Profile Draft Review；Apply 前要求输入 `YES` 并再次确认。前端不会上传简历，也不能提交完整 profile 覆盖数据库，不包含真实发送或招聘平台操作。
...
```powershell
.\.venv\Scripts\python.exe scripts\apply_profile_draft.py
```

确认应用时使用 `--apply`，并按提示输入 `YES`。写入前脚本会把旧 profile 备份到私有 `docs/input/generated/profile_backups/`。保存并读回验证成功后，脚本会写入一条独立的 `profile_apply_history`，只记录 draft / backup 路径、确认与验证标记，以及项目数量和文本长度等轻量摘要，不保存完整 `resume_text` 或 `project_context`。这是本地 Demo 级人工确认与简单留痕，不是自动简历更新流程、完整 profile version management 或生产级 audit log。
...
面试时间回复现在基于手动维护的 `interview_availability_slots`。默认 available 列表会过滤开始时间已经过去的 slots，Agent 不会推荐历史时间；`status=all` 仍可用于查看历史记录。没有可用 slots 时，系统只能生成“需要先确认日程，稍后回复”的草稿；有可用 slots 时，只能提供这些时间段供 HR 参考。系统仍然不会自动预订或确认面试时间，也不会自动发送 HR 消息。
...
Demo 主线统一使用 `8002` 端口，避免和其他项目端口冲突：
...
- `GET /profile_draft/review`：本机读取默认私有 draft 的结构化摘要、预览和文本长度；不存在时返回 `draft_exists=false`，不返回完整正文
- `POST /profile_draft/apply`：本机仅在 `confirmation_text=YES` 时备份、应用、验证并记录最小 history；不接受文件路径或完整 profile
...
- Step 27C：Apply Candidate Profile Draft 已完成，默认 dry-run，写入前备份并要求明确确认。
- Step 27.5：Demo 收口与安全边界修复已完成。`--apply` 只能在交互终端中由用户手动输入 `YES`；默认 available slots 不再返回已经开始的历史时间。
- Step 28A：Profile Apply History 已完成。dry-run、取消和验证失败不会写 history；成功记录的 `external_action_performed` 始终为 false。

当前仍是用于求职展示和本地验证的 Human-in-the-loop Demo，不是生产级自动求职 Agent；不会自动投递、发送 HR 消息或确认面试。
- Step 28：后续可设计更完整的 approval record 或 checkpoint / resume。
