...
当前核心能力仍以规则版 baseline、最小 LangGraph preview 和只读 LLM enhance 为主；除 `/application_review/llm_enhance` 在配置 API key 后会调用 DeepSeek-compatible LLM 外，其他主链路仍不调用 LLM。当前不实现 RAG、Embedding、Playwright 或前端；不连接真实招聘平台，不自动投递，不自动发送 HR 消息，不自动确认面试时间，不自动修改 application status。
...
- Step 15：可选 HR reply draft enhancement，仍然需要人工确认。
