...
当前核心能力仍以规则版 baseline、LangGraph workflow preview、只读 LLM enhance 和可人工审核的 HR reply draft 为主；相关 LLM 接口在配置 API key 后会调用 DeepSeek-compatible LLM。当前不实现 RAG、Embedding、Playwright 或前端；不连接真实招聘平台，不自动投递，不自动发送 HR 消息，不自动确认面试时间。只有用户明确调用 Step 17 确认接口后，系统才更新 application 内部状态。
...
- Step 22：可选的明确 approval record 或 checkpoint / resume 设计，仍不启用外部发送。
