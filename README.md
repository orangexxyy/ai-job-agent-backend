...
现阶段的 `/hr/analyze` 使用本地规则进行意图识别；`/hr/reply` 基于规则、`candidate_profile`、可选的 `applications` 投递记录，以及可选的 `resume_text` / `project_context` 片段生成保守回复草稿。当前接口不真实调用 DeepSeek / LLM，也不消耗真实 API token。
...
评分总分为 100，当前规则包含四个维度：

- `role_fit`：岗位方向是否贴近 AI应用开发 / 大模型应用开发
- `tech_stack_fit`：JD 是否命中 Python、FastAPI、RAG、Agent、LLM、Workflow 等技术栈
- `project_relevance`：JD 是否和 `candidate_profile.available_projects` 中的项目经历相关
- `preference_fit`：是否符合地点、远程/混合办公等偏好，并识别外包、驻场、长期出差、996 等风险信号

当 `update_application=true` 时，接口只会安全回写：
...
如果没有可用上下文，接口会返回保守 fallback，提示补充 `resume_text / project_context`，不会编造项目经历。无论是否命中上下文，回复仍然必须由用户确认后才能发送。
