...
当前核心能力仍以规则版 baseline、最小 LangGraph preview、只读 LLM enhance 和可人工审核的 HR reply draft 为主；相关 LLM 接口在配置 API key 后会调用 DeepSeek-compatible LLM。当前不实现 RAG、Embedding、Playwright 或前端；不连接真实招聘平台，不自动投递，不自动发送 HR 消息，不自动确认面试时间，不自动修改 application status。
...
项目中保留 DeepSeek-compatible 配置。当前只有 `/application_review/llm_enhance` 会在配置 `DEEPSEEK_API_KEY` 后尝试调用 DeepSeek-compatible Chat Completions；没有 API key 时会返回 `llm_used=false` / `api_key_missing`。
...
- Step 16：可选的人工确认后状态更新 workflow，只有用户确认后才更新 application status / next_action。
- Step 17：把 application_review / llm_enhance / hr_reply_draft 接入 LangGraph workflow
- Later：Playwright dry-run 岗位采集，必须人工确认，且不做自动投递。

除 `/application_review/llm_enhance` 外，当前主链路仍不调用 DeepSeek / LLM；当前不做 RAG / Embedding，不做 Playwright，不连接真实招聘平台，不自动投递，不自动发送 HR 消息。
