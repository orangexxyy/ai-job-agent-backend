...
- `scripts/build_profile_draft.py`：读取标准简历文本，规则提取事实并用 `CandidateProfileInput` 校验草稿。
- `docs/profile_draft_builder_design.md`：记录事实链、规则提取、保守默认值和人工审核边界。
- `docs/input/generated/candidate_profile_draft.json`：私有 profile JSON 草稿。
- `docs/input/generated/candidate_profile_draft_report.md`：私有提取与审核报告。

生成文件继续被 `.gitignore` 保护，不属于 Agent 运行时读取路径。

## Step 27C 新增文件

- `scripts/apply_profile_draft.py`：校验私有 draft，默认 dry-run；确认后备份旧 profile 并复用 `profile_service` 写入。
- `docs/input/generated/profile_backups/`：正式应用前的私有 profile JSON 备份目录。

该脚本是人工确认的本地维护工具，不是 Agent Workflow node，也不提供自动更新 API。

## Step 28A: Profile Apply History Files

| File | Responsibility |
| --- | --- |
| `app/models.py` | 定义独立的 `profile_apply_history` SQLite 表 |
| `app/database.py` | 幂等初始化 history 表并显式关闭初始化连接 |
| `app/schemas/profile_apply_history_schema.py` | 定义轻量 `ProfileApplyHistoryItem` |
| `app/services/profile_apply_history_service.py` | 写入和查询 profile apply 留痕；限制 detail 白名单并拒绝外部动作 |
| `scripts/apply_profile_draft.py` | 仅在人工确认、保存和读回验证成功后写入一条 history |
| `scripts/test_profile_apply_history.py` | 使用合成草稿和临时 SQLite 验证写入条件与隐私边界 |

该 history 不复用 application 维度的 action history，不保存完整简历正文，不新增 API，也不是生产级 audit log 或 profile version management。

## Step 29A: Profile Draft Review Files

| File | Responsibility |
| --- | --- |
| `app/schemas/profile_draft_schema.py` | 定义 review 摘要、确认请求和 apply 结果，不包含完整正文提交字段 |
| `app/services/profile_draft_service.py` | 共享 draft 校验、预览、备份、保存、验证和最小 history 流程 |
| `app/routes/profile_draft_routes.py` | 提供本机 `GET /review` 与确认式 `POST /apply`，限制 loopback client 且不接收路径参数 |
| `scripts/apply_profile_draft.py` | 保留 TTY + 手动 `YES`，复用共享 service |
| `scripts/test_profile_draft_review.py` | 使用临时 draft / backup / SQLite 验证 API 和隐私边界 |
| `frontend_demo/index.html` | 增加结构化审核、双重确认和 apply 结果展示 |

HTTP route 固定读取项目默认私有 draft；客户端不能传文件路径、上传简历或提交完整 profile。缺失 draft 以 `draft_exists=false` 返回；该入口不属于 Agent 外部动作链路。
