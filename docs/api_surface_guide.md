...
Step 23 不新增 API。完整演示入口是 `scripts/agent_workflow_demo.py`，它按顺序调用现有 `POST /agent/reply_send_gate/simulate` 和 `GET /applications/{application_id}/action_history`，验证 Step 18-22 链路与安全边界。
