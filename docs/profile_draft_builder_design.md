...
HTTP API 不接受 `draft_path`：GET 拒绝 query 参数，POST schema 只允许 `confirmation_text`。route 仅允许 loopback client，测试显式使用 `127.0.0.1`，不引入账号或权限系统。

该 API 仅用于本机 Demo，不具备认证、多用户隔离、完整版本管理或生产级 audit 能力。它不调用 LLM，不执行投递、HR 消息发送、面试确认或招聘平台操作。
