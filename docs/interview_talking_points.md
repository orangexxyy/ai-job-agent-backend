...
> 我把生成草稿和状态更新拆开，草稿只是 AI 建议，`/application_review/hr_reply_draft` 始终保持只读。只有用户审核并明确调用确认接口后，系统才把 application 更新为 `hr_replied`，并记录下一步动作和人工发送渠道。这样可以避免 LLM 生成内容直接变成现实动作，也不会把 `safe_to_send` 误解为系统已经发送。
...
Step 17 已完成用户确认后的 application 状态更新。后续可以按 Step 18-20 推进：先做错误处理与 retry policy，再设计 checkpoint / resume / approval interrupt，最后补 review history / audit log。
